...
When the star is no longer spherically symmetric, the oscillation equations are no longer separable in :math:`\theta` and :math:`r`.
...
When perturbing the oscillation equations with this term, one finds (e.g. `Aerts et al. 2010 <https://ui.adsabs.harvard.edu/abs/2010aste.book.....A/abstract>`_)

.. math::

   \omega_{\rm rot} = \omega_0 + m \int_0^R K(r) \Omega(r) dr\,,

where :math:`K(r)` depends on the structure of the mode and describes its sensitivity to the rotation rate at radius :math:`r`.
...
This results in frequencies that are similar to :math:`\omega_{\rm rot}` as describe above.
...
First, we parametrize the deformed star with the coordinates :math:`\zeta` and :math:`\theta`, where :math:`\zeta = 0` is the center of the star, and :math:`\zeta = 1` is the surface of the star.
...
The variable :math:`\varepsilon` represents the shift of the isobars due to the rotation, such that the isobars are at constant values of :math:`\zeta`.
We assume that properties of the rotating star that depend on the radius, such as density and pressure, now only depend on :math:`\zeta`.
The only variable that then needs to be determined is :math:`\varepsilon`.
We use the Chandrasekhar-Milne expansion for this.
It defines :math:`\varepsilon` as a series of even Legendre polynomials:

.. math::

	 \varepsilon(\zeta, \theta) = \sum_{k \geq 0} \varepsilon_k(\zeta) P_{2k}(\theta) \,.

StORM includes the :math:`P_0` and :math:`P_2` term.
...
Additionally, only modes with either even or odd degree need to be included, as rotation only couples with :math:`\ell \pm 2` if the deformation is limited to the :math:`P_2` term as described above.
...
The eigenvalues and eigenvectors of this equations give us the perturbed oscillation frequencies and the parameters :math:`a_{kk'}` defined above.
...
In this case we compute zonal modes of :math:`\ell = 0` and :math:`\ell = 2`.
...
- ``deformation-alpha``, ``deformation-dalpha``, and ``deformation-ddalpha``: these define the :math:`P_0` deformed structure of the star, and correspond to :math:`\alpha(a)` in Eq. (27) of `Lee and Baraffe (1995) <https://ui.adsabs.harvard.edu/abs/1995A&A...301..419L>`_
- ``deformation-beta``, ``deformation-dbeta``, and ``deformation-ddbeta``: these define the :math:`P_2` deformed structure of the star, and correspond to :math:`\beta(a)` in Eq. (27) of `Lee and Baraffe (1995) <https://ui.adsabs.harvard.edu/abs/1995A&A...301..419L>`_
- ``deformed-frequency``: Perturbed oscillation frequencies
- ``deformed-eigenvector``: The parameters :math:`a_{kk'}`
...
- Use the parameters :math:`a_{kk'}` to determine which non-perturbed mode contributes most to the perturbed oscillation mode.
