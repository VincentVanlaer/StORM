...
impl Model for StellarModel {
    ...
    fn eval(&self, idx: usize) -> DimensionlessProperties {
        if idx == 0 {
            ...
            let r_cubed = self.r_coord[idx].powi(3);
            DimensionlessProperties {
                v_gamma: self.grav * self.m_coord[idx] * self.rho[idx]
                    / (self.p[idx] * self.r_coord[idx] * self.gamma1[idx]),
                a_star: r_cubed / (self.grav * self.m_coord[idx]) * self.nsqrd[idx],
                u: 4. * PI * self.rho[idx] * r_cubed / self.m_coord[idx],
                c1: r_cubed / self.radius.powi(3) * self.mass / self.m_coord[idx],
                rot: self.rot[idx],
            }
        }
    }
